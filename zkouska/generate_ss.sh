...
# Používáme process substitution < <(...), aby se proměnné zachovaly mimo cyklus
while IFS=';' read -r C_SAMPLE_ID C_RPB2_COUNT C_EIF3E_COUNT C_PHASE_COUNT C_MAX_COPIES rest; do
    #Proměnná rest na konci slouží jako "odpadkový koš" pro případné další sloupce navíc, aby nerozbily čtení.
    ...
    # Znamená: "Pokud je název vzorku prázdný (např. prázdný řádek na konci souboru), přeskoč zbytek tohoto kola a jdi na další řádek (continue)."
    
    # Zjistíme skutečné maximum reálných kopií, které vzorek má (např. 2 a 3 -> 3)
    C_MAX_REAL=$(( C_RPB2_COUNT > C_EIF3E_COUNT ? C_RPB2_COUNT : C_EIF3E_COUNT ))

    # CYKLUS NYNÍ ZAČÍNÁ OD 'MAX_REAL' MÍSTO OD '1'
    for (( TESTED_PLOIDY=C_MAX_REAL; TESTED_PLOIDY<=C_MAX_COPIES; TESTED_PLOIDY++ )); do
        
        OUTPUT_FILE_PATH="${OUTPUT_DIR}/ss_${C_SAMPLE_ID}_${TESTED_PLOIDY}tips.Rev"
        echo "Generuji skript pro: $SAMPLE_ID (Testovaná ploidie: $TESTED_PLOIDY) -> $OUTPUT_FILE_PATH"

        # ----------------------------------------------------------------------
        # 1. LOGIKA PRO DOPLNĚNÍ BLANK TAXA
        # ----------------------------------------------------------------------
    ...
    if [ "$C_PHASE_COUNT" -gt 1 ]; then
        ...
        # ----------------------------------------------------------------------
        # 3. LOGIKA PRO KOMBINATORICKÉ MOVES (Závislá na TESTED_PLOIDY)
        # ----------------------------------------------------------------------
        CORE_MOVES=""
        if [ "$TESTED_PLOIDY" -eq 2 ]; then
            CORE_MOVES+="    moves[++mvi] = mvHomeologPhase(ctmc[i], \"${SAMPLE_ID}_A\", \"${SAMPLE_ID}_B\", weight=2)\n"
        elif [ "$TESTED_PLOIDY" -gt 2 ]; then
            for (( x=1; x<=TESTED_PLOIDY; x++ )); do
                for (( y=x+1; y<=TESTED_PLOIDY; y++ )); do
                    CORE_MOVES+="    moves[++mvi] = mvHomeologPhase(ctmc[i], \"${SAMPLE_ID}_${LETTERS[$x]}\", \"${SAMPLE_ID}_${LETTERS[$y]}\", weight=2)\n"
                done
            done
        fi

        # ======================================================================
        # Zápis do .Rev souboru (Zde zůstává 'cat << EOF' stejný jako předtím)
        # ======================================================================
        
        # ... (zbytek kódu s 'cat << EOF > "$OUTPUT_FILE_PATH"' ...)
...
# ==============================================================================
# 1. ZPRACOVÁNÍ TARGET VZORKŮ (Hlavní cyklus)
# ==============================================================================
# Změna IFS na středník a přidání PHASE_COUNT
while IFS=';' read -r SAMPLE_ID RPB2_COUNT EIF3E_COUNT PHASE_COUNT MAX_COPIES rest; do
    ...
    for (( TESTED_PLOIDY=1; TESTED_PLOIDY<=MAX_COPIES; TESTED_PLOIDY++ )); do
        ...
        if [ "$RPB2_COUNT" -lt "$MAX_COPIES" ]; then
            ...
            if [ "$DIFF" -eq 1 ]; then
                BLANK_LINES_RPB2="\ndata[1].addMissingTaxa(\"${SAMPLE_ID}_BLANK\")"
            else
                for (( j=1; j<=DIFF; j++ )); do
        ...
        if [ "$EIF3E_COUNT" -lt "$MAX_COPIES" ]; then
            ...
            if [ "$DIFF" -eq 1 ]; then
                BLANK_LINES_EIF3E="\ndata[2].addMissingTaxa(\"${SAMPLE_ID}_BLANK\")"
            else
                for (( j=1; j<=DIFF; j++ )); do
