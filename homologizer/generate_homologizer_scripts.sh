...
# ==============================================================================
# PROCESSING OF TARGET SAMPLES (Combined into variables)
# ==============================================================================
TARGET_BLANK_RPB2=""
TARGET_BLANK_EIF3E=""
...
echo "Loading first $NUM_TARGET_TO_TAKE target samples..."

# Načteme pouze určený počet řádků pomocí `head -n`
while IFS=';,' read -r SAMPLE_ID RPB2_COUNT EIF3E_COUNT PHASE_COUNT MAX_COPIES rest; do
    ...
    PHASE_COUNT=$(echo "$PHASE_COUNT" | tr -d '\r' | xargs) # Používáme pevně PHASE_COUNT z tabulky

    if [ -z "$SAMPLE_ID" ]; then continue; fi

    echo " -> Adding: $SAMPLE_ID (number of phases: $PHASE_COUNT)"
...
done < <(tail -n +2 "$TARGET_CSV" | head -n "$NUM_TARGET_TO_TAKE")


# ======================================================================
# GENERATING SINGLE .REV FILE
# ======================================================================
OUTPUT_FILE_PATH="${OUTPUT_DIR}/h_${NUM_TARGET_TO_TAKE}_targets.Rev"
echo "Generating single combined script: $OUTPUT_FILE_PATH"
...
#
bayes_factors = FALSE
output_file = "output/h_${NUM_TARGET_TO_TAKE}_targets"

# input sequence alignments 
alignments = ["nexus_files_samples/RPB2_${NUM_TARGET_TO_TAKE}targets.nex",
              "nexus_files_samples/EIF3E_${NUM_TARGET_TO_TAKE}targets.nex"]
...
# --- MISSING TAXA FOR TARGET SAMPLES ---$(echo -e "$TARGET_BLANK_RPB2")$(echo -e "$TARGET_BLANK_EIF3E")
...
for (i in 1:num_loci) {
    # Core Samples Moves
$(echo -e "$CORE_MOVES")
    # Dynamic Moves for Target Samples
$(echo -e "$TARGET_MOVES")
}
